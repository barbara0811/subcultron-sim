...
launchFileTemplate = "apad_standard_simulation_raw.launch"
...
def generate_yaml_files(n, positions, first_index):
    ...
    for i in range(n):
        ...
        fileOut.write("inertia_matrix: [1, 1, 0.61]\n")
        fileOut.write("added_mass: [3.2, 3.2, 0.74, 0, 0, 0]\n")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print "USAGE: python apad_random_generator.py apad_number [first_index] [north_min] [north_max] [east_min] [east_max]"
        sys.exit(0)
    ...
    tree = xml.etree.ElementTree.parse(rospack.get_path('apad') + '/launch/simulation/' + launchFileTemplate)
    root = tree.getroot()
